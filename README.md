...
message = VatInfo::Message.new({ celk_trzba: '0.00',
                             dic_popl: 'CZ00000019',
                             id_pokl: 'p1',
                             id_provoz: '11',
                             porad_cis: '1' })
...
message.pkp = VatInfo::Utils.create_pkp(message, certificate)
message.bkp = VatInfo::Utils.create_bkp(message.pkp)
```

To sign the message:
```ruby
signed_message = VatInfo::Utils.sign(message.to_xml, certificate)
```

And finally, to send a message:
```ruby
sender = VatInfo::Sender.new
